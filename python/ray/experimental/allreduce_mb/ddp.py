...
@ray.remote
class MLPActor:
    def __init__(self, model_dims: List[int], batch_size: Optional[int] = 8):
        ...
        for i in range(len(model_dims) - 1):
            model_layers.append(
                nn.Linear(model_dims[i], model_dims[i + 1], bias=False).to(self.device)
            )
        ...
        for layer in self.model:
            logger.warning(
                f"Layer size: {layer.weight.numel() * layer.weight.element_size() / 1024**2 :.2f} MiB"
            )
    ...
    def backward(self, bucket: Bucket, t: torch.Tensor) -> Tuple[torch.Tensor, ...]:
        ...
        if bucket.is_last_baucket:
            ...
            logger.warning(
                f"{last_layer_idx_in_bucket}:{self.intermediates[last_layer_idx_in_bucket].shape}, {last_layer_idx_in_bucket+1}:{self.intermediates[last_layer_idx_in_bucket+1].shape}"
            )
            self.intermediates[last_layer_idx_in_bucket].backward(
                self.intermediates[last_layer_idx_in_bucket]
            )
...
def main():
    ...
    actors = [
        MLPActor.options(num_gpus=1).remote(model_dims) for _ in range(num_actors)
    ]
    ...
    with InputNode() as inp:
        ...
        for bucket in buckets[::-1]:
            actors_grads = [
                actor.backward.bind(bucket, res) for actor, res in zip(actors, bw_res)
            ]
            actors_grads = allreduce.bind(actors_grads)
            bw_res = [
                actor.apply_grads.bind(bucket, actor_grads)
                for actor, actor_grads in zip(actors, actors_grads)
            ]
        dag = MultiOutputNode(
            [actor.update.bind(res) for actor, res in zip(actors, bw_res)]
        )
